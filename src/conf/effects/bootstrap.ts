...
export const checkRoomSetting =
  ({ ui, room }: RootStore): EffectCallback =>
  () => {
...
export const initAudioDeviceAndClient =
  ({ ui, client, media }: RootStore): EffectCallback =>
  () => {
    ...
    (async () => {
      ...
      if (audioInDevices.length === 0) {
        throw ui.showError(
          new Error("At least one audio input device needed!")
        );
      }
...
export const listenStoreChanges =
  ({ client, media, room, notification }: RootStore): EffectCallback =>
  () => {
...
export const listenGlobalEvents =
  ({ media, ui }: RootStore): EffectCallback =>
  () => {
